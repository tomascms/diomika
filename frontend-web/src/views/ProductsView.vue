...
import { getCategoryById } from '@/lib/catalogSupabase'
...
watchDynamicTitle(
  () => [categoryData.value?.nome, route.params.categoryId],
  () => {
...
const fetchProducts = async () => {
  try {
    ...
    const cat = supabaseConfigured
      ? await getCategoryById(route.params.categoryId)
      : await apiGet(`/categorias/${route.params.categoryId}`)
    categoryData.value = {
      ...cat,
...
watch(() => route.params.categoryId, fetchProducts)
...
onMounted(async () => {
  ...
  if (supabaseConfigured) {
    productsSubscription = supabase.channel('catalog_realtime')
    for (const table of catalog.realtimeTables()) {
      productsSubscription.on('postgres_changes', { event: '*', schema: 'public', table }, fetchProducts)
    }
    ...
    <div v-else-if="products.length > 0" class="page-shell page-shell--grid product-grid">
      <RouterLink
        ...
        :to="{
          ...
          query: { tipo: product._tipo_catalogo },
        }"
