...
const isCatalogRecord = computed(() =>
  [
    'categories',
    'modelos_almofadas',
    'modelos_assentos',
...
const loadModelAlturas = async (modelId) => {
  if (table.value !== 'assento' || !modelId) {
    fieldOptions.value = { ...fieldOptions.value, altura_modelo: [] }
    return
  }
  try {
    const model = await api.getRecord('modelos_assentos', modelId)
    const raw = model?.alturas
    const alturas = Array.isArray(raw)
      ...
      : typeof raw === 'string'
        ...
        : []
    fieldOptions.value = { ...fieldOptions.value, altura_modelo: alturas }
  } catch {
    fieldOptions.value = { ...fieldOptions.value, altura_modelo: [] }
  }
}
...
const load = async () => {
  ...
  try {
    ...
    await loadModelAlturas(formData.value.id_modelo)
  } catch (e) {
